...
- **交互监控（monitor）**：不带子命令直接运行即进入实时终端；`-tui` 为对话式布局（上方日志、底部固定输入行，↑/↓ 翻阅历史命令，←/→/Home/End 移动光标、退格/Delete 删除，历史跨会话保存于 `.rttsh/tui-history.json`）；输出重定向时转为接收 N 毫秒后退出（默认 500 ms），可接管道过滤。
